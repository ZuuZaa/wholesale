...
const mainFunc=async () => {
  ...
  const fetchData = async () => {
      ...
      let response=await fetch(`https://api.wscshop.co.uk/api/checkout/get-payment`,{
          method: 'GET',
          dataType: 'json',
          headers: {
...
export default function Checkout() {
    ...
    if (addressForm) {
        ShippingAddressBtn = (
           <div>
             <span className='w-9 h-9 flex items-center justify-center rounded-full bg-black hover:bg-gray-600 text-white text-base cursor-pointer' onClick={toggleAddressForm}> x </span>
           </div>
          );
        ...
        ShippingAddressBtn = (
            <div>
             <span className='w-9 h-9 flex items-center justify-center rounded-full bg-black hover:bg-gray-600 text-white text-base cursor-pointer' onClick={toggleAddressForm}> + </span>
            </div>
          );
    }
   ...
   // Radio Buttons
    ...
    const [selectedShippingAddressOption, setSelectedShippingAddressOption] = useState(null);
...
const [data, setData] = useState({ 
  ...
  useEffect(() => {
    ...
    fetchDataAsync();
    let payment="";
    if (typeof localStorage !== 'undefined' && localStorage.getItem("payment") !== null) {
              ...
              <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                  <div className="flex items-center">
                    <a href="/" className="ms-1 text-sm font-medium md:ms-2">Checkout</a>
                  </div>
                </li>
          ...
          <div className='frd-container mx-auto'>
                <div className='checkout-flex grid grid-cols-1 lg:grid-cols-2 gap-8'>
                    ...
                    <div className='checkout-left'>
                            ...
                            {/* Contact */}
                            <h1 className='text-center md:text-left text-xl md:text-3xl font-bold mb-10 border-b pb-3'>Contact</h1>
                            ...
                            <div className='checkout-contact-info-box border rounded-md p-4 mb-14'>
                                <div className='flex mb-4'>
                                    <h5>Contact: </h5>
                                    {
                                      user.map(us=>{
                                        return(
                                          <h6> {us.firstName} {us.lastName}, {us.email}, {us.phone} </h6>
                                        )
                                      })
                                    }
                            ...
                            {/* Shipping */}
                            <h1 className='text-center md:text-left text-xl md:text-3xl font-bold my-10 border-b pb-3'>Delivery</h1>

                            <div className='border rounded-md mb-14 flex items-center justify-center overflow-hidden'>
                                <div className='p-4 w-1/2 relative flex gap-3 border-r border-gray-200' style={{ backgroundColor: selectedShippingOption === "1" ? '#e52e06' : '#f7f7f7' }}>
                                    <input
                                    type="radio"
                                    id="shippingoption1"
                                    ...
                                    className='opacity-0'
                                    />
                                    <label htmlFor="shippingoption1" className='option_layer absolute inset-0 cursor-pointer'></label>
                                    <label htmlFor="shippingoption1" className='text-lg' style={{ color: selectedShippingOption === "1" ? '#fff' : '#333' }}>Delivery</label>
                                </div>

                                <div className='p-4 w-1/2 relative flex gap-3 border-r border-gray-200' style={{ backgroundColor: selectedShippingOption === "2" ? '#e52e06' : '#f7f7f7' }}>
                                    <input
                                    type="radio"
                                    id="shippingoption2"
                                    name="shippingoptions"
                                    ...
                                    className='opacity-0'
                                    />
                                    <label htmlFor="shippingoption2" className='option_layer absolute inset-0 cursor-pointer'></label>
                                    <label htmlFor="shippingoption2" className='text-lg' style={{ color: selectedShippingOption === "2" ? '#fff' : '#333' }}>Collection</label>
                                </div>
                            ...
                            {selectedShippingOption === "1" && (
                                <div className='shippingmethod_wrapper'>
                                    ...
                                    {/* Address */}
                                    <h1 className='text-center flex gap-5 justify-between md:text-left text-xl md:text-2xl font-bold my-10 border-b pb-3 text-red-500'>Delivery Address 
                                    ...
                                    {/* Address Items */}
                                    {
                                      userAddress.map(address=>{
                                        return(
                                          <div className='shipping-address-items p-5 border rounded-md my-6' style={{ border: selectedShippingAddressOption == address.id ? '2px solid #333' : '1px solid #e5e7eb' }}>
                                            <div className='title flex gap-3 items-center'>
                                                <input type='radio'                                      
                                                    id='shp-address-item-1' 
                                                    name='shippingaddressopt' 
                                                    value={address.id}
                                                    checked={selectedShippingAddressOption == address.id}
                                                    onChange={handleShippingAddressOptionChange}
                                                    className='' />
                                                <label htmlFor='shp-address-item-1'>{address.title}</label>
                                            </div>
                            ...
                            {/* Payment */}
                            <h1 className='text-center md:text-left text-xl md:text-3xl font-bold mb-10 border-b pb-3'>Payment</h1>
                            <p className='text-center md:text-left mb-3'>All transactions are secure and encrypted.</p>
                            
                            <div className='border rounded-md mb-14 flex items-center justify-center overflow-hidden'>
                                <div className='p-4 w-1/2 relative flex gap-3 border-r border-gray-200' style={{ backgroundColor: selectedOption === "option1" ? '#e52e06' : '#f7f7f7' }}>
                                    <input
                                    type="radio"
                                    id="option1"
                                    name="options"
                                    ...
                                    className='opacity-0'
                                    />
                                    <label htmlFor="option1" className='option_layer absolute inset-0 cursor-pointer'></label>
                                    <label htmlFor="option1" className='text-lg' style={{ color: selectedOption === "option1" ? '#fff' : '#333' }}>Card</label>
                                </div>

                                <div className='p-4 w-1/2 relative flex gap-3' style={{ backgroundColor: selectedOption === "option2" ? '#e52e06' : '#f7f7f7' }}>
                                    <input
                                    type="radio"
                                    id="option2"
                                    name="options"
                                    ...
                                    className='opacity-0'
                                    />
                                    <label htmlFor="option2" className='option_layer absolute inset-0 cursor-pointer'></label>
                                    <label htmlFor="option2" className='text-lg' style={{ color: selectedOption === "option2" ? '#fff' : '#333' }}>Cash/Bank Transfer</label>
                                </div>
                            </div>
                    ...
                    <div className='checkout-right'>
                        <div className='sticky top-24'>
                            <h1 className='text-center md:text-left text-xl md:text-3xl font-bold mb-3 border-b pb-3'>Order Summary</h1>
                                        ...
                                        return(
                                          <div className='checkout-product-item flex items-center gap-3 mb-5'>
                                              <div className='img border rounded-md relative'>
                                                <img src={product.productImage} width={80} height={80} className='' alt='Product Image'></img>
                                                  <div className='count absolute -top-3 -right-3 w-6 h-6 rounded-full bg-black text-white flex items-center justify-center text-sm'> {product.quantity} </div>
                                              </div>
                                              <div className='name'>
                                                  <h5 className='font-semibold mb-2'>{product.productName}</h5>
                                ...
                                <div className='prices-wrapper pt-7 border-t'>
                                    <div className='flex justify-between'>
                                        <h6 className='font-semibold mb-4 text-lg'>Subtotal</h6><span>£{subtotal}</span>
                                    </div>
                                    <div className='flex justify-between'>
                                        <h6 className='font-semibold mb-4 text-lg'>Discount</h6><span>£{discount}</span>
                                    </div>
                                    <div className='flex justify-between'>
                                        <h6 className='font-semibold mb-4 text-lg'>Delivery Fee</h6><span>£0</span>
                                    </div>
                                </div>
                                ...
                                <div className='prices-wrapper pt-7 border-t'>
                                    <div className='flex justify-between'>
                                        <h6 className='font-semibold mb-4 text-lg'>Total</h6><span className='font-semibold text-lg'>£{total}</span>
                                    </div>
                                    {/* <div className='flex justify-between'>
                                    ...
                                    {selectedOption === "option2" && (
                                        <Link href='javascript:void(0)' className="link-design1 font-bold inline-flex rounded-md gap-1 mt-4 p-4" onClick={cashPayment}>Order Now</Link>
                                    )}
