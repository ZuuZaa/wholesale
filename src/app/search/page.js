...
async function fetchData(){
  ...
  params.append('SessionId', session_id);
    ...
    const response=await fetch(`https://api.wscshop.co.uk/api/search/get-index?${params.toString()}`,{
      method: "GET",
      headers: {
...
export default function Search() {
  ...
  let productFiltering=async()=>{
    ...
    try{
      const res = await fetch("https://api.wscshop.co.uk/api/search/filtering", {
          method: "POST",
          headers: {
            'Accept': 'application/json, text/plain',
            'Content-Type': 'application/json;charset=UTF-8',
              ...
              <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                ...
                <li>
                  <div className="flex items-center">
                    <a href="#" className="ms-1 text-sm font-medium md:ms-2">Search</a>
                  </div>
                </li>
