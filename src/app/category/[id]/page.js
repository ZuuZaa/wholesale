...
export default function Category() {
  ...
  async function fetchData(){
    ...
    params.append('SessionId', session_id);
      ...
      const response=await fetch(`https://api.wscshop.co.uk/api/category/get-index?${params.toString()}`,{
        method: "GET",
        headers: {
...
let productSorting=async(event)=>{
  ...
  try{
    const res = await fetch("https://api.wscshop.co.uk/api/category/sorting", {
        method: "POST",
        headers: {
          'Accept': 'application/json, text/plain',
          'Content-Type': 'application/json;charset=UTF-8',
...
let productFiltering=async()=>{
  ...
  try{
    const res = await fetch("https://api.wscshop.co.uk/api/category/filtering", {
        method: "POST",
        headers: {
          'Accept': 'application/json, text/plain',
          'Content-Type': 'application/json;charset=UTF-8',
                            ...
                            <div className='cf-categories-list'>
                              {
                                mainCategory.map(cat=>{
                                  return(
                                  <div class='active'>
                                    <Link href=''><h3 className='hover-red text-lg font-semibold capitalize'>{cat.name}</h3></Link>
                                    <div className='pl-6'>
                                      {
                                        subCategory.map(subcat=>{
                                          return(<Link href=''><h6 className='hover-red mt-2 font-medium hover:underline capitalize'>{subcat.name}</h6></Link>)
                              ...
                              {/* {
                                mainCategories.map(cat=>{
                                  return(
                                    <div>
                                      <Link href=''><h3 className='hover-red mt-6 text-lg font-semibold capitalize'>{cat.name}</h3></Link>
                                      <div className='pl-6'>
                                        {
