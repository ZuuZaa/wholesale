...
const CounterInput = ({ initialValue, min, max, id }) => {
  ...
  const handleIncrement = async (event) => {
    if (count < max) {
      ...
      const quant = count + 1;
      let price =
        event.currentTarget.parentElement.parentElement.previousSibling.textContent.split(
          "£"
        )[1];
      let total = price * quant;
      event.currentTarget.parentElement.parentElement.nextSibling.textContent =
        "£" + total.toFixed(2);
      try {
