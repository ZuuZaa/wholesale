...
export default function ProductDetail() {
  ...
  async function fetchData(){
    ...
    params.append('SessionId', session_id);
      ...
      const response=await fetch(`https://api.wscshop.co.uk/api/details/get-index?${params.toString()}`,{
        method: "GET",
        headers: {
          'Accept': 'application/json, text/plain',
          'Content-Type': 'application/json;charset=UTF-8',
  ...
  async function fetchData1(){
    ...
    params.append('Id',id)
      ...
      const response=await fetch(`https://api.wscshop.co.uk/api/details/get-desc?${params.toString()}`,{
        method: "GET",
        headers: {
          'Accept': 'application/json, text/plain',
          'Content-Type': 'application/json;charset=UTF-8',
...
//   }, []);
  ...
  let updateInputDetails= (event) => {
    ...
    let quantity=event.currentTarget.value;
    let cartid=event.currentTarget.nextSibling.getAttribute('id')
    event.currentTarget.parentElement.innerHTML='<input class="cart_quant" type="number" min="1" max="10000" value="'+quantity+'" id="'+prodid+'"  /><button type="button" class="rounded-full font-bold inline-block text-base update_button" name="update_button" id="'+cartid+'"><span>Update</span></button>'
  ...
  let updateCartDetails= async (event) => {
    ...
    try {
      ...
      const resJson = await res.json();
        ...
        update_button.parentElement.innerHTML='<input class="cart_quant cart_quant_update" type="number" min="1" max="10000" value="'+quantity+'" id="'+prodid+'" name="cart_quant_update" /><div class="flex gap-2 pm-wrap minus_plus_btn" id="'+cartid+'"><button class="rounded-full font-bold inline-block text-base minus_button" id="'+prodid+'" name="minus_button"><span>-</span></button><button class="rounded-full font-bold inline-block text-base plus_button" id="'+prodid+'" name="plus_button"><span>+</span></button></div>'
        var plus_btns=document.getElementsByName("plus_button")
        //for (let i = 0; i < plus_btns.length; i++) 
        //{
          plus_btns[0].addEventListener('click',plusCartDetails)
  ...
  let submitReview = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch("https://api.wscshop.co.uk/api/details/add-review", {
        method: "POST",
        headers: {
          'Accept': 'application/json, text/plain',
          'Content-Type': 'application/json;charset=UTF-8'
                ...
                <div className='custom-container mx-auto'>
                    ...
                    <ol className="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                        ...
                        <div className="flex items-center">
                            <a href="#" className="ms-1 text-sm font-medium md:ms-2">  </a>
                        </div>
                        </li>
                    </ol>
                    </div>
            ...
            <section className='product-main-section my-20 py-3'>
                ...
                {products.map(product=>{
                    ...
                    const fav_icon=[];
                      ...
                      {
                        add_cart_div.push(<div className='add-to-cart-wrap pm flex items-center justify-center gap-2'><input class="cart_quant cart_quant_update" type="number" min="1" max="10000" defaultValue={product.quantity} onKeyUp={updateInputDetails} id={product.id}  /><div className='flex gap-2 pm-wrap minus_plus_btn' id={product.cartId}><button type="button" className='rounded-full font-bold inline-block text-base minus_button' onClick={minusCartDetails} id={product.id}><span>-</span></button><button type="button" className='rounded-full font-bold inline-block text-base plus_button' onClick={plusCartDetails} id={product.id}><span>+</span></button></div></div>)

                      }
                      else{
                        add_cart_div.push(<div className='add-to-cart-wrap flex items-center justify-center gap-2'><input class="cart_quant" type="number" min="1" max="10000" defaultValue="1"/><button type='button' onClick={addCartDetails} id={product.id} className='rounded-full font-bold inline-block text-base add_cart_btn'>Add</button></div>)
                      }
                      
                      let price_div=[];
                      if(userType==1){
                        price_div.push(<div className='product-detail-price-box'><h5>Wholesale Price</h5><h3 className='product-price text-red-500 text-2xl font-semibold'>£{product.wholeSalePrice.toFixed(2)}</h3> </div>)
                      }
                      else if(userType==2){
                        price_div.push(<div className='product-detail-price-box'><h5>Wholesale Price</h5><h3 className='product-price text-red-500 text-2xl font-semibold'>£{product.wholeSalePrice.toFixed(2)}</h3> </div>)
                        price_div.push(<div className='product-detail-price-box'><h5>Diller Price</h5><h3 className='product-price text-red-500 text-2xl font-semibold'>£{product.dillerPrice.toFixed(2)}</h3> </div>)
                      }
                      else if(userType==3){
                        price_div.push(<div className='product-detail-price-box'><h5>Wholesale Price</h5><h3 className='product-price text-red-500 text-2xl font-semibold'>£{product.wholeSalePrice.toFixed(2)}</h3> </div>)
                        price_div.push(<div className='product-detail-price-box'><h5>Diller Price</h5><h3 className='product-price text-red-500 text-2xl font-semibold'>£{product.dillerPrice.toFixed(2)}</h3> </div>)
                        price_div.push(<div className='product-detail-price-box'><h5>Special Price</h5><h3 className='product-price text-red-500 text-2xl font-semibold'>£{product.specialPrice.toFixed(2)}</h3> </div>)
                      }
                      const limit_div=[];
                      if(product.stock<=0){
                        limit_div.push(<h2 className='product-availability text-base mb-3 text-red-500'>Not available</h2>)
                      }
                      else if(product.stock<=product.minLimit){
                        limit_div.push(<h2 className='product-availability text-base mb-3 text-red-500'>Limited number</h2>)
                      }
                    return(
                    <section className='custom-products-section pb-6'>
                        <div className='frd-container mx-auto mb-10'>
                            <div className='grid grid-cols-1 lg:grid-cols-2'>
                                ...
                                <div className='product-detail-info px-4'>
                                    ...
                                    {
                                      userType>0&&product.stock>0?(<h4 className='product-availability text-base mb-3 text-stone-800'>Availability: <span className='text-green-400 '>{product.stock} in stock</span></h4>):(<h4></h4>)
                                    }
                                    {limit_div}
                                    ...
                                    <p className='product-paragraph text-md text-gray-600 mt-6'>{product.about}</p>
                                    <h4 className='product-sku text-base mt-6 text-stone-800 flex items-center gap-9'>Code: {product.code} 
                                            <div className='rating flex justify-center items-center '>
                                                {items}
                                                <span className='text-sm ml-1 text-slate-700'>{reviews.length} review</span>
                                            </div>
                                    </h4>
                                    <div className='product-price-wrap flex gap-5 py-4 mt-6 mb-6 items-center'>
                                        <h3 className='product-price text-red-500 text-2xl font-semibold'>£{product.price.toFixed(2)}</h3>
                      ...
                                    </div>):(<div></div>)
                                    ...
                                    <div id='prod_desc' className='mb-6'></div>
                                    {
                                      ...
                                      <Link className='text-sm text-red-600 mb-3' href={product.dahuaLink} target='blank'>You can find Datasheet of product by clicking here</Link>
                                    ...
                                    <div className='product-add-wrap flex items-center gap-4'>
                                      <div className='add-to-cart' style={{margin:"0"}}>
                                        ...
                                        {fav_icon}
                                        <div className='addtowishlist-btn link-design1-cancel hover-red-bg py-2 px-3 rounded-md' onClick={addCompare} id={product.id}>
                                    ...
                                    <div className='product-cards mt-6'>
                                        <p className='text-sm text-gray-600 mb-3'>Guaranteed safe checkout</p>
                                        <div className='flex'>
                              ...
                              <div className="py-4">
                                  <div className='product-detail-review-wrap py-6 px-2 md:p-6 pd-review-border'>
                                      ...
                                      {reviews.length>0?(<div className='product-detail-review-top'>
                                          <h5 className='text-lg text-gray-700 font-semibold mb-3'>CUSTOMER REVIEWS</h5>   
                                          {products.map(product=>{
                                              const items = [];
                                              ...
                                              <div className='rating flex justify-center items-center gap-2'>
                                                  {items}
                                                  <span className='text-sm ml-1 text-slate-700'>Based on {reviews.length} review</span>
                                              </div>
                                              {
                                                isLogin==1?(<div style={{cursor:"pointer"}}>
                                                <span className='hover-red text-sm' onClick={toggleVisibility}>Write a review</span>
                                            </div>):(<div style={{cursor:"pointer"}}>
                                                  <Link href="/login" className='hover-red text-sm'>Please login for write a review</Link>
                                              </div>)
                                              }
                                          ...
                                          <h5 className='text-lg text-gray-700 font-semibold mb-3'>CUSTOMER REVIEWS</h5>   
                                              ...
                                              <div className='rating flex justify-center items-center gap-2'>
                                                  ...
                                                  <UilStar size="18" color="#ffc400"></UilStar>
                                                  <UilStar size="18" color="#ffc400"></UilStar>
                                                  <span className='text-sm ml-1 text-slate-700'>No reviews yet</span>
                                              </div>
                                              {
                                                isLogin==1?(<div style={{cursor:"pointer"}}>
                                                <span className='hover-red text-sm' onClick={toggleVisibility}>Write a review</span>
                                      ...
                                      {isVisible && (
                                        <div className='product-detail-review-from'>
                                          <form className='py-4' onSubmit={submitReview}>
                                              ...
                                              <label className='text-sm text-slate-500 mt-4 block mb-1'>Rating</label>
                                              ...
                                              <label className='text-sm text-slate-500 mt-4 block mb-1'>Review Title</label>
                                              <input className="appearance-none block w-full  border rounded-sm py-3 px-4  leading-tight" 
                                                  type="text" 
                                                  placeholder="Give your review a title" onChange={(e) => setTitle(e.target.value)} />

                                              <label className='text-sm text-slate-500 mt-4 block mb-1'>Body of Review</label>
                                              <textarea className="appearance-none block w-full  border rounded-sm py-3 px-4  leading-tight" 
                                                  type="text" 
                                                  placeholder="Write your comments here" onChange={(e) => setText(e.target.value)} />
                                              <div className='flex justify-end'>
                                                  <button type="submit" className='link-design1-cancel hover-red-bg py-4 px-8  md:px-12 lg:px-16 rounded-3xl my-3 text-base'>Submit Review</button>
                                              </div>
